...
async function api(url, options = {}) {
  ...
  try {
    while (true) {
      ...
      // Passwordless mode obtains a signed, HttpOnly session automatically.
      // The browser never sees or stores the operator secret.
      try {
        const bound = await fetch('/api/login', { method: 'POST', credentials: 'same-origin', headers: { 'Content-Type': 'application/json' }, body: '{}' });
        if (!bound.ok) break;
      } catch { break; }
...
async function loadLeaderboard({ refresh = false, silent = false } = {}) {
  ...
  try {
    ...
    // A restored browser snapshot can contain verified rows from before the
    // target route's short-lived P0 receipts were issued. Refresh once for the
    // same route instead of leaving every checkbox permanently disabled.
    const incomingHasReceipt = requestSource === 'catalog' && (body.books || []).some((book) => Boolean(book.p0Receipt));
    if (requestSource === 'catalog' && !refresh && !incomingHasReceipt && body.dataQuality === 'verified_metrics' && state.leaderboardReceiptRefreshKey !== requestKey) {
      state.leaderboardReceiptRefreshKey = requestKey;
      $('#leaderboardUpdated').textContent = '正在为当前账号刷新 P0 校验收据…';
      setTimeout(() => loadLeaderboard({ refresh: true, silent: true }), 0);
    }
