...
function reserveModelFor(modelChoice) {
  ...
  // DeepSeek V4 is a real primary route; never silently route it to HY3.
...
function hydrateCreativeEvidence(value, section, evidenceBank = []) {
  ...
  if (section === 'videoPrompt' && value && typeof value === 'object') {
    const hydrated = { ...value, sourceEvidence: hydrateList(value.sourceEvidence || value.source_evidence) };
    if (hydrated.scenePlan && typeof hydrated.scenePlan === 'object') {
      const ids = new Set((hydrated.sourceEvidence || []).map((item) => String(item?.evidenceId || '')).filter(Boolean));
      hydrated.scenePlan = { ...hydrated.scenePlan, evidenceIds: Array.isArray(hydrated.scenePlan.evidenceIds)
        ? hydrated.scenePlan.evidenceIds.map((id) => String(id || '')).filter((id) => !ids.size || ids.has(id))
        : [] };
    }
    return hydrated;
  }
...
function normalizeCreativeWireObject(value) {
  ...
  const aliases = {
    ...
    scenePlan: ['sceneplan', 'scene', 'shotplan', 'shot_plan'],
    adCopy: ['adcopy'],
...
The videoPrompt is a high-retention vertical short-video story package, not generic visual prose: it must use supplied chapter facts to provide a 0-2s hook, 2-5s personal stake/value promise, 5-8s escalation, 8-11s reversal, and 11-15s cliffhanger. The reversal must be a genuine plot turn from evidence, never invented. Give each beat a chapter and exact short quote, then write compelling ${languageLabel} narration and an explicit ${languageLabel} 0-15s shot plan with character lock. Also provide natural Chinese operator translations. Prohibit subtitles, readable text, CTA cards and identity drift in the generated video. Create two distinct concise English image prompts plus Chinese translations: luminous_cinema 9:16 using nano, and editorial_romance 2:3 using gpt. Image prompts must show one decisive supported moment, reserve negative space, and prohibit readable text, title, logo, watermark, QR, UI, collage, duplicated people and extra limbs. Finally, assess the finished creative package as a production editor. Give only a concise operator-facing conclusion, not private reasoning: recommendation must be keep or refine; choose refine only when a specific source-grounded improvement would materially improve the hook, story logic, video reversal, or visual moment. Explain why and name the target.\n\nSelected creative strategy:\n${styleGuidance}${revisionInstruction}`;
  const schema = {
    ...
    videoPrompt: { hook: '0-3s source-grounded conflict', valuePromise: 'personal stake within the opening, not an extra shot', escalation: '3-6s source-grounded escalation', reversal: '6-12s source-supported shift', cliffhanger: '9-12s unresolved ending', scenePlan: { scene: 'one source-grounded location and time', cast: [{ role: 'protagonist', name: 'exact supplied name', anchor: 'stable adult appearance, wardrobe and posture' }, { role: 'counterpart', name: 'exact supplied name', anchor: 'stable adult appearance, wardrobe and posture' }], props: ['one evidence-grounded conflict object'], lighting: 'motivated source light direction and tactile palette', timeBeats: [{ time: '0-3s', shot: 'size, camera and continuous motion', action: 'one feasible action', reaction: 'visible physical reaction', sound: 'diegetic cue', dialogue: 'optional short attributed line' }, { time: '3-6s', shot: 'size, camera and continuous motion', action: 'one feasible action', reaction: 'visible physical reaction', sound: 'diegetic cue', dialogue: 'optional short attributed line' }, { time: '6-9s', shot: 'size, camera and continuous motion', action: 'one feasible action', reaction: 'visible physical reaction', sound: 'diegetic cue', dialogue: 'optional short attributed line' }, { time: '9-12s', shot: 'size, camera and continuous motion', action: 'unfinished supported choice', reaction: 'visible reaction', sound: 'diegetic cue', dialogue: 'optional short attributed line' }], negative: ['no subtitles', 'no readable text', 'no logos', 'no watermark', 'no identity drift'], evidenceIds: ['exact supplied ID', 'different supplied ID', 'third supplied ID'] }, sourceEvidence: [{ evidenceId: 'exact supplied ID' }, { evidenceId: 'different supplied ID' }, { evidenceId: 'third supplied ID' }], adCopy: `${languageLabel} complete 12-second four-block scene prompt compiled from scenePlan`, buildRequirement: `${languageLabel} concise execution summary compiled from scenePlan`, zhHook: 'Chinese translation', zhValuePromise: 'Chinese translation', zhEscalation: 'Chinese translation', zhReversal: 'Chinese translation', zhCliffhanger: 'Chinese translation', zhAdCopy: 'Chinese narration translation', zhBuildRequirement: 'Chinese shot plan translation', evidenceChapters: [1, 2] },
  ...
  // Video prompt v2: dialogue-led scene direction, distilled from the
  // operator's 2026-09-11 research. Keep the existing wire schema and evidence IDs.
...
SCHEMA: hook summarizes 0-3s; valuePromise describes the personal stake shown there (not an extra timed shot); escalation summarizes 3-6s; reversal describes the supported shift across 6-12s; cliffhanger is the unresolved 9-12s ending. Include scenePlan with scene, cast, props, lighting, negative and exactly four timeBeats. Every timeBeat must contain time, shot, action, reaction and sound and must describe one feasible action in the same continuous location. Keep cast to the two named adult roles unless evidence requires a third. Include exactly 3 sourceEvidence objects containing only different supplied evidenceId values. Chinese operator translations belong only in zh fields. Return only the compact required JSON object.`;
  ...
  const compactSectionSource = {
    book: {
      ...
      description: String(book?.description || '').replace(/\s+/g, ' ').slice(0, 1600),
