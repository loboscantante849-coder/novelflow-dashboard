...
function request(method, token, body = {}) {
  return { method, headers: { authorization: `Bearer ${token}`, host: 'attacker.example' }, body, query: body };
}

function seedPromotion() {
  FakeRedis.reset({
    'nf_user_subs:alice': new Set(['1000']),
    nf_subs: { 1000: JSON.stringify({
...
test('QR promotion card is only created for a server-owned promotion and returns a self-hosted QR bitmap', async () => {
  seedPromotion();
  const result = await invoke(qrPromotion, request('POST', signAccessToken({ username: 'alice' }), { code: '1000' }));
...
test('QR promotion rejects another account and never trusts a supplied redirect URL', async () => {
  seedPromotion();
  const result = await invoke(qrPromotion, request('POST', signAccessToken({ username: 'mallory' }), { code: '1000', destination: 'https://evil.example' }));
...
test('a QR scan increments a counter then performs a temporary redirect without exposing the record', async () => {
  seedPromotion();
  const asset = await invoke(qrPromotion, request('POST', signAccessToken({ username: 'alice' }), { code: '1000' }));
