...
test('stats consumers flag multiple approved wallets for one source instead of failing', async () => {
  ...
  assert.equal(summary.statusCode, 200);
  assert.equal(summary.body.income_review_required, true);
  assert.equal(links.statusCode, 200);
  assert.equal(links.body.income_review_required, true);
});
