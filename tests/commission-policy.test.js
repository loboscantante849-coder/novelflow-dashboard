...
test('complete daily data applies 100 percent before cutoff and 80 percent from cutoff', () => {
  ...
  assert.equal(balances.commission_income, 124);
  assert.equal(balances.total_earned, 127);
  assert.equal(balances.available_balance, 97);
  assert.equal(balances.withdrawable_balance, 97);
  ...
  assert.equal(balances.promotion_income_total, 124);
