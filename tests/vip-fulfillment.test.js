...
function pendingEvent(overrides = {}) {
  return {
    version: 1, event_id: 'vip_test_event', username: 'vip-worker-user', user_id: USER_ID,
    source: 'limited_subsidy', source_id: 'limited-subsidy-v1', days: 2, status: 'pending',
    attempts: 0, created_at: '2026-08-11T00:00:00.000Z', metadata: {}, ...overrides,
  };
}

function workerRequest() {
  return { method: 'POST', headers: { authorization: `Bearer ${process.env.CRON_SECRET}` } };
}
...
test.after(() => { require.cache[memberPath].exports = originalMemberExports; });

test('VIP worker grants a pending event exactly once', async () => {
  const event = pendingEvent();
  FakeRedis.values.set(`nf_vip_event:v1:${event.event_id}`, JSON.stringify(event));
  let grants = 0;
  grantVip = async () => { grants += 1; return { success: true, request_id: 'request-success' }; };
  ...
  assert.equal(JSON.parse(FakeRedis.values.get(`nf_vip_event:v1:${event.event_id}`)).status, 'succeeded');
});

test('upstream failure requires reconciliation and is not retried', async () => {
  const event = pendingEvent();
  FakeRedis.values.set(`nf_vip_event:v1:${event.event_id}`, JSON.stringify(event));
  let grants = 0;
  grantVip = async () => { grants += 1; const error = new Error('timed out'); error.code = 'UPSTREAM_TIMEOUT'; throw error; };
...
test('delivering events are not blindly retried', async () => {
  const event = pendingEvent({ status: 'delivering', attempts: 1 });
  FakeRedis.values.set(`nf_vip_event:v1:${event.event_id}`, JSON.stringify(event));
  let grants = 0;
  grantVip = async () => { grants += 1; return { success: true }; };
  const response = await invoke(vipFulfillment, workerRequest());
  assert.equal(response.statusCode, 200);
